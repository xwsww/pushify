...
from services.deploy_logs import supplement_logs
            ...
            while True:
                ...
                try:
                    logs = await request.app.state.loki_service.get_logs(
                        ...
                        start_timestamp=str(logs_start_timestamp),
                        limit=5000,
                        timeout=10.0,
                    )
                ...
                if not logs:
                    async with AsyncSessionLocal() as db:
                        fresh = await db.get(Deployment, deployment.id)
                    if fresh:
                        deployment.container_id = fresh.container_id
                        deployment.status = fresh.status
                        deployment.conclusion = fresh.conclusion
                    fallback = await supplement_logs(deployment, [])
                    logs = [
                        log
                        for log in fallback
                        if int(log["timestamp"]) >= logs_start_timestamp
                    ]
                ...
                if logs:
                    new_logs = [
                        log
                        for log in logs
                        if log["message"] not in emitted_messages
                    ]
                    batch_max = max(int(log["timestamp"]) for log in logs)
                    if new_logs:
                        for log in new_logs:
                            emitted_messages.add(log["message"])
                        logs_html = logs_template.module.log_list(logs=new_logs)
                        last_id = max(int(log["timestamp"]) for log in new_logs)
                        yield f"id: {last_id}\n"
                        yield "event: deployment_log\n"
                        yield f"data: {logs_html.replace(chr(10), '').replace(chr(13), '')}\n\n"
                    logs_start_timestamp = batch_max + 1
