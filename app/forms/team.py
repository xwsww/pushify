...
class TeamMemberAddForm(StarletteForm):
    ...
    async def async_validate_email(self, field):
        ...
        if not account:
            raise ValidationError(
                _(
                    "No account with this email. The person must sign in with GitHub first, then you can invite them."
                )
            )
...
class TeamInviteDeclineForm(StarletteForm):
    invite_id = HiddenField(validators=[DataRequired()])
    submit = SubmitField(_l("Decline"))


class TeamInviteRevokeForm(StarletteForm):
    invite_id = HiddenField(validators=[DataRequired()])
    email = HiddenField(validators=[DataRequired(), Email()])
    confirm = StringField(
        _l("Confirmation"),
        validators=[DataRequired(), Email(message=_l("Invalid email address"))],
    )
    submit = SubmitField(_l("Revoke invite"))

    def validate_confirm(self, field):
        if field.data.strip().lower() != (self.email.data or "").strip().lower():  # type: ignore
